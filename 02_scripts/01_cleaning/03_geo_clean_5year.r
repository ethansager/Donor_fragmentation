...
build_annual_afro <- function(
  ...
  filled <- lapply(by_unit, function(unit_df) {
    ...
    observed_years <- unit_df$year[!is.na(unit_df$wave) & is.finite(unit_df[[sgq_col]])]
...
bucket_afro <- function(
  ...
  annual_afro %>%
    ...
    summarise(
      ...
      afro_count = safe_mean(afro_count),
      wave = safe_max(wave),
      sgq_status_observed_share = mean(sgq_status_year == "observed", na.rm = TRUE),
      sgq_status_interpolated_share = mean(sgq_status_year == "interpolated", na.rm = TRUE),
      sgq_status_backfilled_share = mean(sgq_status_year == "backfilled_pre2005", na.rm = TRUE),
      sgq_status_carry_forward_share = mean(sgq_status_year == "carry_forward_post2015", na.rm = TRUE),
      sgq_status = {
    ...
    mutate(
      sgq_status_observed_share = if_else(is.nan(sgq_status_observed_share), NA_real_, sgq_status_observed_share),
      sgq_status_interpolated_share = if_else(is.nan(sgq_status_interpolated_share), NA_real_, sgq_status_interpolated_share),
      sgq_status_backfilled_share = if_else(is.nan(sgq_status_backfilled_share), NA_real_, sgq_status_backfilled_share),
      sgq_status_carry_forward_share = if_else(is.nan(sgq_status_carry_forward_share), NA_real_, sgq_status_carry_forward_share)
    )
}
...
raw_dat <- read_csv(here("00_rawdata", "GODAD_projectlevel.csv"), show_col_types = FALSE)
...
admin1_pop <- read_csv("00_rawdata/population/admin1_population.csv", show_col_types = FALSE) %>%
  pivot_longer(
...
admin2_pop <- read_csv("00_rawdata/population/admin2_population.csv", show_col_types = FALSE) %>%
  pivot_longer(
    cols = starts_with("sum_"),
    names_to = "year",
    values_to = "pop"
...
if (length(sector_admin1_list) > 0) {
  ...
  write_csv(
    ...
    here("01_panel_data", paste0("panel_aid_admin1_by_sector", PANEL_TAG, ".csv"))
  )
}
...
if (length(sector_admin2_list) > 0) {
  ...
  write_csv(
    ...
    here("01_panel_data", paste0("panel_aid_admin2_by_sector", PANEL_TAG, ".csv"))
  )
}
