...
emdat <- openxlsx::read.xlsx(
  "00_rawdata/public_emdat_custom_request_2025-02-17_c70922f5-cb0d-4162-ad6f-83809e87c7c0.xlsx"
)
...
countries_iso3 <- read_csv(
  "00_rawdata/nightlights/topcodefix/processed_topcodefix_nl_admin1.csv"
) %>%
  select(GID_0) %>%
  distinct() %>%
  pull()
...
write.csv(
  admin1_panel_with_disasters,
  "01_panel_data/panel_aid_admin1_with_disasters.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  "01_panel_data/panel_aid_admin2_with_disasters.csv",
  row.names = FALSE
)
