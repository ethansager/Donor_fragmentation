...
The repository now includes a redesign-first analysis module in
`02_scripts/03_rebuild/` with explicit estimands, confirmatory vs exploratory
separation, and reproducible outputs.

Run:

- `make rebuild-redesign` (sequential pipeline)
- `make rebuild-targets` (targets DAG; requires the `targets` package)
