...
hhi_results_admin2 <- dat %>%
  ...
  summarise(total_disb_admin2 = sum(abs(disb_loc_evensplit), na.rm = TRUE), .groups = "drop") %>%
...
hhi_results_admin1 <- dat %>%
  ...
  summarise(total_disb_admin1 = sum(abs(disb_loc_evensplit), na.rm = TRUE), .groups = "drop") %>%
...
table(unique(panel_aid_admin1$GID_0))
