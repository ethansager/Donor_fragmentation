...
panel_aid <-  read_csv("01_panel_data/panel_aid_admin2.csv")%>%
    filter(GID_0 %in% study_countries & paymentyear ==2015)
...
africa_admin1 <- sf::st_make_valid(
  sf::st_as_sf(geodata::gadm(country = african_iso3_ssa, level = 1, path = "00_rawdata/shapefiles/"))
)
...
ggplot(country_avgs, aes(x = country_mean_fragmentation, y = country_mean_sgq)) +
    geom_smooth(method = "lm", color = "blue", se = TRUE) +
    geom_point(size = 3, color = "black", alpha = 0.7) +
    geom_text(aes(label = GID_0), vjust = -0.5, size = 3) +
    labs(
